...
export default function Home() {
  ...
  const handlePhotos = async (files: FileList | null) => {
    ...
    // Límite de 4 fotos máximo
    const remainingSlots = 4 - editing.images.length;
    if (remainingSlots <= 0) {
      showToast('⚠️ Máximo 4 fotos por producto');
      return;
    }
    const filesToProcess = Array.from(files).slice(0, remainingSlots);
    
    const uploaded: string[] = [];
    showToast('Procesando foto(s)...');
    ...
    if (uploaded.length) showToast(`${uploaded.length} foto(s) subidas`);
  };
    ...
    return (
      <>
        ...
        <main style={{ padding: '20px 16px 100px', display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <input type="text" placeholder="Nombre o alias del cliente" value={posClient} onChange={e => setPosClient(e.target.value)} style={inputStyle} />
              ...
              {editing.images.length < 4 && (
                <>
                  <label style={{ width: 76, height: 76, borderRadius: 12, border: '1.5px dashed var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold-deep)', fontSize: 24, cursor: 'pointer', background: 'var(--nude-soft)', flexShrink: 0, flexDirection: 'column', gap: 2 }}>
                    ...
                    <input type="file" accept="image/*" capture="environment" style={{ display: 'none' }} onChange={e => handlePhotos(e.target.files)} />
                  </label>
